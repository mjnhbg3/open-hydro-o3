...
class KPICalculator:
    ...
    async def calculate_current_kpis(
        self, sensor_data: Dict[str, Any], target_ranges: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            ph_in_spec = self._is_in_range(
                water.get("ph"), target_ranges["ph_min"], target_ranges["ph_max"]
            )

            ec_in_spec = self._is_in_range(
                water.get("ec"), target_ranges["ec_min"], target_ranges["ec_max"]
            )

            temp_in_spec = self._is_in_range(
                air.get("temperature"),
                target_ranges["temp_min"],
                target_ranges["temp_max"],
            )

            humidity_in_spec = self._is_in_range(
                air.get("humidity"),
                target_ranges["humidity_min"],
                target_ranges["humidity_max"],
            )

            co2_in_spec = self._is_in_range(
                air.get("co2"), target_ranges["co2_min"], target_ranges["co2_max"]
            )
    ...
    async def calculate_period_kpis(self, period_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            temp_values = [
                r["air_temp"] for r in sensor_data if r["air_temp"] is not None
            ]
            humidity_values = [
                r["humidity"] for r in sensor_data if r["humidity"] is not None
            ]
                ...
                kpis.update(
                    {
                        ...
                        "ph_std": round(
                            statistics.stdev(ph_values) if len(ph_values) > 1 else 0, 3
                        ),
                        "ph_in_spec_pct": round(
                            self._calculate_in_spec_percentage(ph_values, 5.5, 6.5), 1
                        ),
                    }
                )
                ...
                kpis.update(
                    {
                        ...
                        "ec_std": round(
                            statistics.stdev(ec_values) if len(ec_values) > 1 else 0, 3
                        ),
                        "ec_in_spec_pct": round(
                            self._calculate_in_spec_percentage(ec_values, 1.2, 2.0), 1
                        ),
                    }
                )
                ...
                kpis.update(
                    {
                        ...
                        "temp_std": round(
                            (
                                statistics.stdev(temp_values)
                                if len(temp_values) > 1
                                else 0
                            ),
                            2,
                        ),
                        "temp_in_spec_pct": round(
                            self._calculate_in_spec_percentage(temp_values, 18, 26), 1
                        ),
                    }
                )
                ...
                kpis.update(
                    {
                        ...
                        "humidity_in_spec_pct": round(
                            self._calculate_in_spec_percentage(humidity_values, 50, 70),
                            1,
                        ),
                    }
                )
                ...
                kpis.update(
                    {
                        ...
                        "co2_in_spec_pct": round(
                            self._calculate_in_spec_percentage(co2_values, 400, 1200), 1
                        ),
                    }
                )
            ...
            for reading in sensor_data:
                if all(
                    ...
                    for key in ["ph", "ec", "air_temp", "humidity", "co2"]
                ):
    ...
    async def calculate_7day_trends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ph_values = [
                k["ph_avg"] for k in kpi_history if k.get("ph_avg") is not None
            ]
            ec_values = [
                k["ec_avg"] for k in kpi_history if k.get("ec_avg") is not None
            ]
            health_values = [
                k["health_score"]
                for k in kpi_history
                if k.get("health_score") is not None
            ]
                ...
                trends.update(
                    {
                        ...
                        "ph_in_spec_7day": round(
                            statistics.mean(
                                [k.get("ph_in_spec_pct", 0) for k in kpi_history]
                            ),
                            1,
                        ),
                    }
                ...
                trends.update(
                    {
                        ...
                        "ec_in_spec_7day": round(
                            statistics.mean(
                                [k.get("ec_in_spec_pct", 0) for k in kpi_history]
                            ),
                            1,
                        ),
                    }
    ...
    def _is_in_range(
        self, value: Optional[float], min_val: float, max_val: float
    ) -> float:
    ...
    def _calculate_in_spec_percentage(
    ...
    def _calculate_dosing_totals(
        ...
        totals["ml_total"] = (
            totals["pump_a_ml"] + totals["pump_b_ml"] + totals["ph_pump_ml"]
        )
