...
class VectorMemory:
    ...
    def _init_chromadb(self):
        ...
        try:
            ...
            self.logger.info(
                f"ChromaDB initialized with {self.collection.count()} memories"
            )
            ...
            if results["documents"] and results["documents"][0]:
                for i, doc in enumerate(results["documents"][0]):
                    ...
                    try:
                        ...
                        stored_data["similarity_score"] = (
                            1 - distance
                        )  # Convert distance to similarity
                        memories.append(stored_data)
                    except json.JSONDecodeError:
                        self.logger.warning("Failed to parse stored memory data")
                        continue
        ...
        try:
            if self.mock_mode:
                ...
                self._mock_memories = [
                    ...
                    if datetime.fromisoformat(m.get("timestamp", "")).timestamp()
                    > cutoff_time
                ]
                return
        ...
        if "timestamp" in memory_data:
            try:
                dt = datetime.fromisoformat(
                    memory_data["timestamp"].replace("Z", "+00:00")
                )
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"mode": "error", "error": str(e), "available": False}
