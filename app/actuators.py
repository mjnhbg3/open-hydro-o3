...
class ActuatorController:
    ...
    def __init__(self, mock: bool = False):
        ...
        self.states = {
            'pumps': {'pump_a': False, 'pump_b': False, 'ph_pump': False, 'refill_pump': False},
            'fan_speed': 0,
            'led_power': 0,
            ...
            try:
                ...
                if success:
                    results['executed'][pump_name] = {
                        'ml': ml_amount,
                        'reason': reason,
                        'duration_seconds': ml_amount / self.FLOW_RATES.get(pump_name, 2.5)
                    }
    ...
    async def control_fan(self, speed_percent: int, duration_minutes: int = None) -> Dict[str, Any]:
        """Control fan speed"""
    ...
    async def control_led(self, power_percent: int, schedule: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    async def _execute_pump_dose(self, pump_name: str, ml_amount: float) -> bool:
        ...
        self.logger.info(f"Dosing {pump_name}: {ml_amount}ml for {duration_seconds:.1f}s")
