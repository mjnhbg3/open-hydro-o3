...
class CameraInterface:
    ...
    async def time_lapse_capture(
        self, interval_minutes: int = 60, duration_hours: int = 24
    ) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Starting time-lapse: {total_captures} images over {duration_hours}h"
        )
        ...
        try:
            for i in range(total_captures):
                ...
                if "error" not in result:
                    captured += 1
                    self.logger.info(
                        f"Time-lapse frame {i+1}/{total_captures} captured"
                    )
                else:
                    self.logger.error(
                        f"Failed to capture frame {i+1}: {result['error']}"
                    )
    ...
    async def detect_growth_changes(
        self, reference_image_path: str, current_image_path: str
    ) -> Dict[str, Any]:
