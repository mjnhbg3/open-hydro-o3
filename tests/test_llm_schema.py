...
class TestLLMSchemaValidation:
    ...
    def test_dose_schema_validation(self):
        ...
        assert validate_json_schema(valid_dose, schema) == True
    ...
    def test_fan_schema_validation(self):
        ...
        assert validate_json_schema(valid_fan, schema) == True
    ...
    def test_led_schema_validation(self):
        ...
        assert validate_json_schema(valid_led, schema) == True
...
class TestLLMAgent:
    ...
    async def test_llm_decision_making(self, llm_agent, mock_sensor_data, mock_config):
        ...
        llm_agent.kpi_calc.calculate_current_kpis = AsyncMock(return_value={
            'health_score': 0.8,
            'ph_in_spec': 0.9,
            'ec_in_spec': 0.85
        })
    ...
    def test_user_message_generation(self, llm_agent, mock_sensor_data):
        ...
        message = llm_agent._prepare_user_message(mock_sensor_data, kpis, recent_actions)
...
class TestLLMIntegration:
    ...
    async def test_llm_with_vector_memory(self, mock_chromadb):
        ...
        with patch.dict('os.environ', {'OPENAI_API_KEY': 'test-key'}):
            with patch('app.llm_agent.VectorMemory') as mock_vector_class:
                ...
                agent.client.chat.completions.create.return_value.choices[0].message.content = json.dumps({
                    "decisions": {"dose": {"pump_a": {"ml": 2.0, "reason": "test"}}},
    ...
    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_llm_decision_consistency(self, llm_agent, mock_sensor_data, mock_config):
