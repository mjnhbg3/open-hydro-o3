    ...
    async def poll_sensors(self) -> dict:
        ...
        try:
            ...
            if not validation_result['valid']:
                self.logger.warning(f"Sensor validation issues: {validation_result['warnings']}")
        ...
        try:
            ...
            if ph is not None:
                if ph < 3.0 or ph > 10.0:
                    ...
                    warnings.append(f"pH {ph} outside typical hydroponic range (4.5-8.0)")
            ...
            if water_temp is not None:
                if water_temp < 0 or water_temp > 40:
                    critical.append(f"Water temp {water_temp}°C outside safe range (0-40°C)")
                elif water_temp < 15 or water_temp > 30:
                    warnings.append(f"Water temp {water_temp}°C outside optimal range (15-30°C)")
            
            air_temp = air.get('temperature')
            if air_temp is not None:
                if air_temp < -10 or air_temp > 50:
                    critical.append(f"Air temp {air_temp}°C outside sensor range (-10-50°C)")
                elif air_temp < 16 or air_temp > 30:
                    warnings.append(f"Air temp {air_temp}°C outside optimal range (16-30°C)")
            
            # Humidity validation
            humidity = air.get('humidity')
            if humidity is not None:
                if humidity < 0 or humidity > 100:
                    critical.append(f"Humidity {humidity}% outside possible range (0-100%)")
                elif humidity > 85:
                    warnings.append(f"High humidity {humidity}% (>85%) - mold risk")
                elif humidity < 30:
                    warnings.append(f"Low humidity {humidity}% (<30%) - plant stress risk")
            ...
            if level_low is False:
                critical.append("Water level critically low - refill needed immediately")
