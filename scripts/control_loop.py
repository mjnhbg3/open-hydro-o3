...
class ControlLoop:
    ...
    async def run_control_cycle(self) -> dict:
        ...
        try:
            ...
            rules_result = await self.rules_engine.evaluate_rules(
            ...
            filtered_decisions = (
                await self.rules_engine.apply_stable_unless_better_logic(
            ...
            execution_results = await self._execute_actions(
                filtered_decisions["filtered_actions"]
            )
            ...
            self.logger.info(
                f"Control cycle completed in {cycle_result['cycle_duration_ms']}ms"
            )
            ...
            if self.db:
                try:
                    ...
                    self.logger.error(
                        f"Failed to store control cycle error: {db_error}"
                    )
    ...
    def _combine_decisions(self, rules_result: dict, llm_result: dict = None) -> dict:
        ...
        combined = {
            ...
            "sources": {
                "rules": True,
                "llm": llm_result is not None and "error" not in llm_result,
            },
        }
        ...
        try:
            ...
            if actions.get("emergency_rollback"):
                self.logger.critical(
                    f"Emergency rollback triggered: {actions['rollback_reason']}"
                )
            ...
            if "fan" in actions:
                ...
                fan_result = await self.actuators.control_fan(
                    fan_data.get("fan_speed", 0), fan_data.get("duration_minutes")
                )
            ...
            if "led" in actions:
                ...
                led_result = await self.actuators.control_led(
                    led_data.get("led_power", 0), led_data.get("schedule")
                )
            ...
            if execution_results["executed"]:
                ...
                if "dosing" in execution_results["executed"]:
                    executed_doses = execution_results["executed"]["dosing"].get(
                        "executed", {}
                    )
                    if executed_doses:
                        dose_desc = [
                            f"{pump} {data['ml']}ml"
                            for pump, data in executed_doses.items()
                        ]
