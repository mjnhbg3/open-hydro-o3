...
class KPIRollup:
    ...
    async def run_kpi_rollup(self) -> dict:
        ...
        try:
            ...
            for period_name, hours in time_periods:
                try:
                    ...
                    if "error" not in period_kpis:
                        ...
                        self.logger.info(f"KPI rollup completed for {period_name}: ID={kpi_id}")
                    else:
                        self.logger.warning(f"KPI calculation failed for {period_name}: {period_kpis['error']}")
                        results["rollups"][period_name] = {"error": period_kpis["error"]}
            ...
            self.logger.info(f"KPI rollup completed in {duration_ms}ms with {len(results['alerts'])} alerts")
